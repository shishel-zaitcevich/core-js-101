...
function getPolynom(...args) {
  return (x) => {
    ...
    while (index < args.length) {
      res += args[index] * x ** (args.length - 1 - index);
      index += 1;
    }
    return res;
  };
...
function logger(func, logFunc) {
  return (...arg) => {
    ...
    const start = `${func.name}(${argString.substring(
      1,
      argString.length - 1,
    )}) starts`;
    logFunc(start);
    const res = func(...arg);
    logFunc(
      `${func.name}(${argString.substring(1, argString.length - 1)}) ends`,
    );
    return res;
  };
}
...
function partialUsingArguments(fn, ...args1) {
  return (...args2) => {
    const res = args1.concat(args2);
    return fn(res[0], res[1], res[2], res[3]);
  };
}
